...
`claude-code/` 目录是一个完整的 [Claude Code 插件](https://code.claude.com/docs/en/plugins)。使用 `--plugin-dir` 加载：
...
插件已捆绑 MCP 服务器配置（`.mcp.json`）和技能 —— 无需额外配置。
...
├── claude-code/                        # Claude Code 插件（原生 MCP）
│   ├── .claude-plugin/
│   │   └── plugin.json                 # 插件清单
│   ├── .mcp.json                       # MCP 服务器配置（已捆绑）
│   └── skills/
│       └── aws-knowledge/
│           ├── SKILL.md                # 技能定义
│           └── references/
